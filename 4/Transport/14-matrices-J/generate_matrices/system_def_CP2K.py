...
- Xatom2ign:   If a projected DOS is to be calculated, indicates which atoms have to be ignore to have the
               isolated molecule for the pDOS
'''
...
def build_and_dump(cfg):
    ...
    iatom2ign        = sdef.SystemDef(cfg).iatom2ign
    fatom2ign        = sdef.SystemDef(cfg).fatom2ign
...
###################################################################
    ...
    # elemC:  is the number of elements in of the CP2K numbering of the central system
...
# Lines to use for getting specific orbitals from the KS and S matrices. Uncomment/modify if necessary
    #elemC    = int(2*100)   
    #elemlL   = int(2*100)   
    #elemlR   = int(2*100)   
...
# Lines to use for getting specific orbitals from the KS and S matrices. Uncomment/modify if necessary
        ...
        np.savetxt(path+"/SC.dat", S[elemlL:elemlL+elemC, elemlL:elemlL+elemC], fmt = '%.5f')
        np.savetxt(path+"/SL.dat", S[elemlL-elemppL:elemlL, elemlL-elemppL:elemlL], fmt = '%.5f')
        np.savetxt(path+"/SR.dat", S[elemlL+elemC:elemlL+elemC+elemppR, elemlL+elemC:elemlL+elemC+elemppR], fmt = '%.5f')
        
        np.savetxt(path+"/SCL.dat", S[elemlL:elemlL+elemC, elemlL-elemppL:elemlL], fmt = '%.5f')
        np.savetxt(path+"/SCR.dat", S[elemlL:elemlL+elemC, elemlL+elemC:elemlL+elemC+elemppR], fmt = '%.5f')
        
        if Nperiods == 1: 
            np.savetxt(path+"/STL.dat",S[elemlL:elemlL+elemC, elemlL-elemppL:elemlL], fmt = '%.5f')
            np.savetxt(path+"/STR.dat",S[elemlL:elemlL+elemC, elemlL+elemC:elemlL+elemC+elemppR], fmt = '%.5f')
        elif Nperiods > 1:
            np.savetxt(path+"/STL.dat",S[elemlL-elemppL:elemlL, elemlL-(2*elemppL):elemlL-elemppL], fmt = '%.5f')
            np.savetxt(path+"/STR.dat", S[elemlL+elemC:elemlL+elemC+elemppR, elemlL+elemC+elemppR:elemlL+elemC+2*(elemppR)], fmt = '%.5f')
            
        np.savetxt(path+"/HC.dat", KS[elemlL:elemlL+elemC, elemlL:elemlL+elemC], fmt = '%.5f')
        np.savetxt(path+"/HL.dat", KS[elemlL-elemppL:elemlL, elemlL-elemppL:elemlL], fmt = '%.5f')
        np.savetxt(path+"/HR.dat", KS[elemlL+elemC:elemlL+elemC+elemppR, elemlL+elemC:elemlL+elemC+elemppR], fmt = '%.5f')
        np.savetxt(path+"/VCL.dat", KS[elemlL:elemlL+elemC, elemlL-elemppL:elemlL], fmt = '%.5f')
        np.savetxt(path+"/VCR.dat", KS[elemlL:elemlL+elemC, elemlL+elemC:elemlL+elemC+elemppR], fmt = '%.5f')
        if Nperiods == 1: 
            np.savetxt(path+"/TL.dat", KS[elemlL:elemlL+elemC, elemlL-elemppL:elemlL], fmt = '%.5f')
            np.savetxt(path+"/TR.dat", KS[elemlL:elemlL+elemC, elemlL+elemC:elemlL+elemC+elemppR], fmt = '%.5f')
        elif Nperiods > 1:
            np.savetxt(path+"/TL.dat", KS[elemlL-elemppL:elemlL, elemlL-(2*elemppL):elemlL-elemppL], fmt = '%.5f')
            np.savetxt(path+"/TR.dat", KS[elemlL+elemC:elemlL+elemC+elemppR, elemlL+elemC+elemppR:elemlL+elemC+2*(elemppR)], fmt = '%.5f')
        ...
        for i,matrix in zip(["alpha","beta"],[KS_alpha,KS_beta]):
            np.savetxt(path+"/HC-"+i+".dat", matrix[elemlL:elemlL+elemC, elemlL:elemlL+elemC], fmt = '%.5f')
            np.savetxt(path+"/HL-"+i+".dat", matrix[elemlL-elemppL:elemlL, elemlL-elemppL:elemlL], fmt = '%.5f')
            np.savetxt(path+"/HR-"+i+".dat", matrix[elemlL+elemC:elemlL+elemC+elemppR, elemlL+elemC:elemlL+elemC+elemppR], fmt = '%.5f')
            np.savetxt(path+"/VCL-"+i+".dat", matrix[elemlL:elemlL+elemC, elemlL-elemppL:elemlL], fmt = '%.5f')
            np.savetxt(path+"/VCR-"+i+".dat", matrix[elemlL:elemlL+elemC, elemlL+elemC:elemlL+elemC+elemppR], fmt = '%.5f')
            if Nperiods == 1: 
                np.savetxt(path+"/TL-"+i+".dat", matrix[elemlL:elemlR+elemC, 0:elemlL], fmt = '%.5f')
                np.savetxt(path+"/TR-"+i+".dat", matrix[elemlR:elemlR+elemC, elemlR+elemC:2*elemlR+elemC], fmt = '%.5f')
            elif Nperiods > 1:
                np.savetxt(path+"/TL-"+i+".dat", matrix[elemlL-elemppL:elemlL, elemlL-2*elemppL:elemlL-elemppL], fmt = '%.5f')
                np.savetxt(path+"/TR-"+i+".dat", matrix[elemlL+elemC:elemlL+elemC+elemppR, \
                               elemlL+elemC+elemppR:elemlL+elemC+2*elemppR], fmt = '%.5f')
    ...
    print ("Entire time for Generating 14 Matrices: {:.1f}:{:.1f}:{:.1f} h/m/s".format(hours,minutes,seconds))
