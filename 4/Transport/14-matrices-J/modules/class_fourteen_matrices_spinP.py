...
class fourteen_matrices_spin:
    ...
    def NEGF(self):
        ...
        for spin in ["alpha","beta"]: 
            ...
            '''
            init DOS and transmission: Each of the arrays has to have the dimensions of the energy points in the
            specific process
            '''
            ...
            '''
            Wait until all of them are finished, then, get the results from every process into the root (master)
            '''
            ...
            if rank == 0:
                np.savetxt(path+"/out-"+spin+".dat", np.c_[all_energy.real, all_dos, all_trans], fmt="%.5f")
                if spin == "alpha":
                    stophalf = time.time()
                    temphalf = stophalf-start
                    hourshalf = temphalf//3600
                    minuteshalf = temphalf//60 - hourshalf*60
                    secondshalf = temphalf - 60*minuteshalf
                    print ("Parallel calculation")
                    print ("System name : ", self.Sname)
                    print ("Time for half of calculation: {:.0f}:{:.0f}:{:.0f} h/m/s".format(hourshalf,minuteshalf,secondshalf))
                elif spin == "beta":
                    stop = time.time()
                    temp = stop-start
                    hours = temp//3600
                    ...
                    print ("Entire time for NEGF method: {:.0f}:{:.0f}:{:.0f} h/m/s \n".format(hours,minutes,seconds))
